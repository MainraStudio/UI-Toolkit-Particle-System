...
///
/// Menus:
/// • GameObject ▸ UI Toolkit ▸ Extract to UIParticle Profile
/// • Assets ▸ Create ▸ UI Toolkit ▸ UIParticle ▸ Extract Profile From Selection Prefabs
/// </summary>
namespace MainraGames.Editor
{
    public static class UIParticleProfileExtractor
    {
        ...
        // Reuse the same logic for both menu entries (Hierarchy + Project)
        ...
        private static void ExtractFromSelection()
        {
            ...
            if (objs == null || objs.Length == 0)
            {
                EditorUtility.DisplayDialog(
                    ...
                    "Select a prefab asset or a GameObject (Hierarchy/Project) that contains a ParticleSystem.",
                    "OK");
                return;
            }
        ...
        /// <summary>
        /// Validator for the Assets (Project window) menu. Enabled only for prefab assets
        /// (Regular, Variant, or Model) that contain at least one ParticleSystem.
        /// </summary>
        [MenuItem("Assets/Create/UI Toolkit/UIParticle/Extract Profile From Selection Prefabs", true)]
        private static bool ExtractFromSelection_ProjectMenu_Validate()
        {
            // Project window: filter to assets only
            ...
            foreach (var go in gos)
            {
                ...
                if (prefabType != PrefabAssetType.NotAPrefab &&
                    go.GetComponentInChildren<ParticleSystem>(true) != null)
                {
                    return true;
                }
            }
        ...
        /// <summary>
        /// Validator for the GameObject (Hierarchy) menu. Enabled when at least one selected
        /// GameObject in the scene hierarchy contains a ParticleSystem.
        /// </summary>
        [MenuItem("GameObject/UI Toolkit/Extract to UIParticle Profile", true)]
        private static bool ExtractFromSelection_HierarchyMenu_Validate()
        {
        ...
        private static UIParticleProfile CreateProfileFromParticleSystem(ParticleSystem ps)
        {
            ...
            // ─────────────────────────────────────────────────────────────────────
            // POSITION & CULLING (no direct counterpart in ParticleSystem)
            // ─────────────────────────────────────────────────────────────────────
        ...
        private static void GetAvgAndRandom(ParticleSystem.MinMaxCurve mmc, out float avg, out float plusMinusRandom)
        {
            ...
            {
                avg = GetConstant(mmc, 0f); // t=0 as representative
                plusMinusRandom = 0f;
            }
        }
